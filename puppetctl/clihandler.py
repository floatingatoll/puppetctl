        ...
        if cfilename:
            ...
            for (section, options) in acceptable_options.items():
                for option in options:
                    try:
                        returndict[option] = cfileparser.get(section, option).encode('utf-8')
                    except (configparser.MissingSectionHeaderError,
 ...
 Duration: +__m, +__h, +__d (minutes, hours, days from now)''')
    ...
    def subcommand_break_all_locks(self, ctlcmd, subcmd, argv):
        ...
        if args.force < 2:
            parser.print_help()
            sys.exit(1)
        # pass along the 'force' count, we'll check it again on the other side
